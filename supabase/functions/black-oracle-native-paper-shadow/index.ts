...
const VERSION = "BO-SUPABASE-NATIVE-SHADOW-v0.1";
...
const POSITIVE = [
  "approval", "approved", "adoption", "inflow", "inflows", "rally", "surge", "record high", "upgrade", "partnership",
  "launch", "buying", "accumulation", "승인", "도입", "유입", "상승", "급등", "채택",
];
const NEGATIVE = [
  "hack", "hacked", "exploit", "lawsuit", "ban", "outflow", "outflows", "liquidation", "crash", "fraud", "investigation",
  "breach", "selloff", "해킹", "소송", "금지", "유출", "청산", "급락", "사기", "조사",
];
...
const parseFeed = (xml: string) => Array.from(xml.matchAll(/<item\b[\s\S]*?<\/item>/gi)).map((match) => {
  const item = match[0];
  const title = field(item, "title");
  const link = field(item, "link") || field(item, "guid");
  const description = field(item, "description");
  const publishedText = field(item, "pubDate") || field(item, "dc:date") || field(item, "published");
  const publishedAt = Date.parse(publishedText);
  return { title, link, description, publishedAt };
}).filter((item) => item.title && /^https?:\/\//i.test(item.link) && Number.isFinite(item.publishedAt));

const countHits = (text: string, words: string[]) => words.reduce((sum, word) => sum + (text.includes(word) ? 1 : 0), 0);

const classify = (text: string) => {
  const positive = countHits(text, POSITIVE);
  const negative = countHits(text, NEGATIVE);
  if (positive === 0 && negative === 0) return { direction: "NEUTRAL" as const, strength: 20, rationale: "No deterministic directional keyword matched; retained as neutral context." };
  if (positive === negative) return { direction: "NEUTRAL" as const, strength: 25, rationale: `Directional keyword conflict (${positive}/${negative}); retained as neutral context.` };
  const direction = positive > negative ? "BULLISH" as const : "BEARISH" as const;
  const hits = Math.max(positive, negative);
  return { direction, strength: Math.min(55, 35 + hits * 5), rationale: `${direction} deterministic keyword evidence (${positive} positive / ${negative} negative hits).` };
};
...
const collectEvidence = async () => {
  ...
  const responses = await Promise.allSettled(FEEDS.map(async (feed) => {
    ...
    try {
      const response = await fetch(feed.url, {
        headers: { accept: "application/rss+xml, application/xml, text/xml, */*", "user-agent": "Black-Oracle-Native-Shadow/0.1" },
  ...
  for (const result of responses) {
    if (result.status === "rejected") {
      warnings.push(result.reason instanceof Error ? result.reason.message : "RSS collection failed.");
      continue;
    }
    ...
    for (const item of items) {
      ...
      for (const asset of ASSETS) {
        ...
        const normalized = buildExternalTradingEvidence({
          market: asset.market,
          title: item.title,
          ...
          tags: ["supabase-native-shadow", "deterministic-classifier"],
        }, {
          relevant: true,
          direction: deterministic.direction,
          strength: deterministic.strength,
          expiryHours: feed.sourceType === "PRIMARY" ? 48 : 18,
...
const resetRuntime = () => {
  ...
  paperLoopController.restore({
    schemaVersion: 1,
    running: false,
    config: { intervalMs: 15 * 60 * 1000, maxMarkets: 6, maxOpenPositions: 4 },
    cycleCount: 0,
    lastCycle: null,
    marketHistory: [],
    cycleHistory: [],
...
Deno.serve(async (req: Request) => {
  ...
  const { data: claimed, error: claimError } = await admin.rpc("claim_black_oracle_trading_cycle_lease", {
  ...
  try {
    ...
    if (row?.checkpoint && typeof row.checkpoint === "object") {
      const checkpoint = row.checkpoint as any;
      paperTradingSession.restore(checkpoint.session);
      tradingEvidenceStore.replaceAll(Array.isArray(checkpoint.evidence) ? checkpoint.evidence : []);
      tradeCaseStore.replaceAll(Array.isArray(checkpoint.tradeCases) ? checkpoint.tradeCases : []);
      paperLoopController.restore(checkpoint.loop, false);
    } else {
      resetRuntime();
    }

    const collected = await collectEvidence();
    const current = tradingEvidenceStore.list(undefined, false);
    const merged = new Map<string, any>(current.map((item: any) => [item.id, item]));
    for (const item of collected.evidence) merged.set(item.id, item);
    tradingEvidenceStore.replaceAll(Array.from(merged.values()).slice(0, MAX_EVIDENCE));
    ...
    const checkpoint = {
      schemaVersion: 1,
      savedAt,
      reason: "supabase-native-shadow-cycle",
    ...
    const { error: saveError } = await admin.from(RUNTIME_TABLE).upsert({
    ...
    return json({
      ...
      authority: { mode: "PAPER_SHADOW", liveTrading: false, productionAuthority: false },
      evidence: { active: tradingEvidenceStore.list().length, collected: collected.evidence.length, warnings: collected.warnings.slice(0, 8) },
      cycle: {
    ...
    await admin.rpc("release_black_oracle_trading_cycle_lease", { p_runtime_id: RUNTIME_ID, p_owner: owner }).catch(() => undefined);
  }
});

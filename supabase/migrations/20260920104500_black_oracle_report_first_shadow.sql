...
create table if not exists public.black_oracle_reports (
  ...
  status text not null check (status in ('DRAFT','REVIEWING','PUBLISHED','CORRECTED','SUPERSEDED','ARCHIVED','BLOCKED')),
...
create table if not exists public.black_oracle_analyst_reviews (
  ...
  foreign key (report_id, report_version)
    references public.black_oracle_reports(report_id, version) on delete restrict
);
...
create table if not exists public.black_oracle_forecast_evaluations (
  ...
  forecast_id text not null references public.black_oracle_report_forecasts(forecast_id) on delete restrict,
...
comment on table public.black_oracle_reports is
  'Versioned Report-first research records. No execution authority.';

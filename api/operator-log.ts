  ...
  try {
    ...
    const chronologicalDecisions = history.flatMap((cycle, index) => {
      ...
      return cycle.markets.map((item) => {
        ...
        const auditCompleteness = assessAuditCompleteness({
          ...
          forecastAvailable: Boolean(item.forecast?.available),
          scenarioLinked: false,
          councilLinked: Boolean(tradeCase?.councilRunId),
          executionLinked,
          outcomeLinked,
        ...
        return {
          ...
          evidenceState: evidenceStateFor(item),
          evidenceActiveCount: item.evidenceActiveCount ?? 0,
          ...
          councilRunId: tradeCase?.councilRunId ?? null,
          intelligencePackageId: tradeCase?.intelligencePackageId ?? null,
          auditCompleteness,
    ...
    for (const item of chronologicalDecisions) {
      ...
      if (collapsible && previous && previous.fingerprint === item.fingerprint && previous.decision === item.decision) {
        ...
        previous.lastCycleNumber = item.cycleNumber;
        previous.timestamp = item.timestamp;
        previous.cycleNumber = item.cycleNumber;
        previous.auditCompleteness = item.auditCompleteness;
        continue;
      }
      const group = {
        ...item,
        ...
        firstCycleNumber: item.cycleNumber,
        lastCycleNumber: item.cycleNumber,
      };
    ...
    for (const item of chronologicalDecisions) {
      const previous = previousEvidenceState.get(item.market);
      if (previous && previous !== item.evidenceState) {
        evidenceTransitions.push({
          cycleNumber: item.cycleNumber,
          timestamp: item.timestamp,
          market: item.market,
          from: previous,
          to: item.evidenceState,
          resolved: item.evidenceState === 'SUPPORTED',
    ...
    return response.status(200).json({
      ...
      decisions,
      monitorDecisions: monitorGroups.slice(-300).reverse(),
      evidenceTransitions: evidenceTransitions.slice(-100).reverse(),

export default async function handler(request: any, response: any) {
  ...
  try {
    ...
    url.searchParams.set('select', 'id,market,timeframe_minutes,bars,candidate_count,seed,started_at,finished_at,status_counts,top_results,execution_authority,promotion_authority');
    ...
    return response.status(200).json({ success: true, available: true, runs, latestRun: runs?.[0] ?? null });
  } catch (error) {

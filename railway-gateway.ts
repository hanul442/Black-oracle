...
let internalServerExited = false;

child.on('exit', (code, signal) => {
  internalServerExited = true;
...
app.get('/health', (_req, res) => {
  const internalServerAlive = !internalServerExited && child.exitCode === null && child.signalCode === null;
  const ok = authConfigured && internalServerAlive;
  const state = !authConfigured
    ? 'AUTH_NOT_CONFIGURED'
    : internalServerAlive
      ? 'SERVING'
      : 'INTERNAL_SERVER_EXITED';

  res.status(ok ? 200 : 503).json({
    ok,
    service: 'black-oracle-web-gateway',
    state,
    authConfigured,
    internalServerAlive,
    ...
    healthSemantics: 'PROCESS_LIVENESS',
    tradingRuntimeHealth: '/api/trading/runtime/health',
  });
});
...
app.get('/api/report-read-shadow', (req, res) => {
  const internalSecret = String(process.env.CRON_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();
  if (!internalSecret) return res.status(503).json({ success: false, error: 'Report read internal authorization is unavailable.' });
  req.headers.authorization = `Bearer ${internalSecret}`;
  void reportReadShadowHandler(req, res);
});
...
app.get('/api/credit-economy-status', (req, res) => {
  const internalSecret = String(process.env.CRON_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();
  if (!internalSecret) return res.status(503).json({ success: false, error: 'Credit economy authorization is unavailable.' });
  req.headers.authorization = `Bearer ${internalSecret}`;
  void creditEconomyStatusHandler(req, res);
});

...
test('future snapshot fails closed while preserving the record for audit', async () => {
  const record = makeRecord('2026-09-21T00:01:00.000Z');
  const repository: UpbitUniverseSnapshotRepository = {
    save: async () => undefined,
    latest: async () => record,
  };

  const result = await readLatestUpbitUniverse(repository, new Date('2026-09-21T00:00:00.000Z'));
  assert.equal(result.record, record);
  assert.equal(result.scannerEligible, false);

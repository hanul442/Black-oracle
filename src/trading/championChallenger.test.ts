...
const champion: StrategyCandidate = {
  id: 'champion-v1', strategyVersion: 'strategy-v1', modelVersion: null, registeredAt: 1,
  role: 'CHAMPION', experimentId: null, regimes: ['UPTREND', 'RANGE'],
};
const challenger: StrategyCandidate = {
  id: 'challenger-v2', strategyVersion: 'strategy-v2', modelVersion: 'model-v2', registeredAt: 2,
  role: 'CHALLENGER', experimentId: 'exp-v2', regimes: ['RANGE'],
};
const passEvidence = { experimentStatus: 'PASSED' as const, monteCarloVerdict: 'PASS' as const, reliabilityVerdict: 'PASS' as const };
...
test('failed validation gates block promotion review', () => {
  const registry = new ChampionChallengerRegistry(champion);
  registry.registerChallenger(challenger);
  const review = registry.assess(challenger.id, { experimentStatus: 'PASSED', monteCarloVerdict: 'WATCH', reliabilityVerdict: 'EXTEND' });
...
test('explicit human approval is required before Champion replacement', () => {
  const registry = new ChampionChallengerRegistry(champion);
  registry.registerChallenger(challenger);
  assert.throws(() => registry.promote(challenger.id, passEvidence, { humanApproved: false, approvedBy: 'human', approvedAt: 3 }), /Human approval/);
  const result = registry.promote(challenger.id, passEvidence, { humanApproved: true, approvedBy: 'Hanseo', approvedAt: 4 });

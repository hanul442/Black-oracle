...
test('Experiment Ledger preserves an append-only lifecycle', () => {
  ...
  ledger.start({ id: 'run-001', experimentId: spec.id, startedAt: spec.createdAt + 100, finishedAt: null, seed: 42, sampleSize: 100, source: 'MONTE_CARLO', status: 'RUNNING' }, spec.createdAt + 100);
...
test('Experiment criteria preserve unavailable metrics explicitly', () => {
  const results = evaluateExperimentCriteria(spec, { maxDrawdownPct: 0.04, expectancy: null });

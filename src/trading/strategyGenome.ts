...
export interface StrategyGenomeMutation { type: StrategyGenomeMutationType; field: string; from: number | string | boolean | null; to: number | string | boolean | null; }
export interface StrategyGenomeWeights { eventNews: number; trendMomentum: number; meanReversion: number; }
export interface StrategyGenomeThresholds { entryScore: number; exitScore: number; minConfidence: number; }
export interface StrategyGenomeRiskProfile { maxPositionPct: number; maxDailyLossPct: number; maxTotalDrawdownPct: number; }
export interface StrategyGenome {
  id: string; generation: number; createdAt: number; parentGenomeIds: string[]; strategyVersion: string; modelVersion: string | null;
  markets: string[]; regimes: string[]; timeframesMinutes: number[]; weights: StrategyGenomeWeights; thresholds: StrategyGenomeThresholds;
  risk: StrategyGenomeRiskProfile; mutations: StrategyGenomeMutation[]; executionAuthority: false;
}

const uniqueSortedStrings = (items: string[], upper = false) => [...new Set(items.map((item) => (upper ? item.trim().toUpperCase() : item.trim())).filter(Boolean))].sort();
...
export const normalizeStrategyGenome = (genome: StrategyGenome): StrategyGenome => {
  ...
  validateThresholds(genome.thresholds); validateRisk(genome.risk);
  return Object.freeze({
    ...genome, id: genome.id.trim(), parentGenomeIds: Object.freeze(uniqueSortedStrings(genome.parentGenomeIds)) as string[], strategyVersion: genome.strategyVersion.trim(),
    modelVersion: genome.modelVersion?.trim() || null, markets: Object.freeze(uniqueSortedStrings(genome.markets, true)) as string[], regimes: Object.freeze(uniqueSortedStrings(genome.regimes, true)) as string[],
    timeframesMinutes: Object.freeze([...new Set(genome.timeframesMinutes)].sort((a, b) => a - b)) as number[], weights: normalizeWeights(genome.weights),
    thresholds: Object.freeze({ ...genome.thresholds }), risk: Object.freeze({ ...genome.risk }),
    mutations: Object.freeze(genome.mutations.map((mutation) => Object.freeze({ ...mutation, field: mutation.field.trim() }))) as StrategyGenomeMutation[], executionAuthority: false,
  });
};
const stableGenomePayload = (genome: StrategyGenome) => ({ strategyVersion: genome.strategyVersion, modelVersion: genome.modelVersion, markets: genome.markets, regimes: genome.regimes, timeframesMinutes: genome.timeframesMinutes, weights: genome.weights, thresholds: genome.thresholds, risk: genome.risk });
const fnv1a32 = (text: string) => { let hash = 0x811c9dc5; for (let index = 0; index < text.length; index += 1) { hash ^= text.charCodeAt(index); hash = Math.imul(hash, 0x01000193) >>> 0; } return hash >>> 0; };
export const fingerprintStrategyGenome = (genome: StrategyGenome) => { const normalized = normalizeStrategyGenome(genome); const hash = fnv1a32(JSON.stringify(stableGenomePayload(normalized))); return `sg-${hash.toString(16).padStart(8, '0')}`; };

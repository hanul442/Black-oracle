...
const Metric = ({ label, value, accent, note }: { label: string; value: React.ReactNode; accent?: string; note?: React.ReactNode }) => (
  <div className="min-w-0">
    ...
    <div className="mt-1 truncate text-[15px] font-semibold tracking-[-0.02em]" style={{ color: accent ?? ink }}>{value}</div>
...
const EquitySparkline = ({ points }: { points: Array<{ timestamp: number; equity: number }> }) => {
  const values = points.slice(-80).map((item) => item.equity).filter(Number.isFinite);
  if (values.length < 2) return <div className="h-20 rounded-[18px] bg-[#f6f7f9]" />;
...
const Header = ({ operations, loading, refresh }: { operations: OperationsPayload | null; loading: boolean; refresh: () => void }) => {
  ...
  return <div className="sticky top-0 z-40 border-b border-[#eceef1] bg-white/95 backdrop-blur-xl">
    <div className="flex items-center justify-between px-5 pb-3 pt-[max(env(safe-area-inset-top),14px)]">
      ...
      <button type="button" onClick={refresh} className="flex h-10 w-10 items-center justify-center rounded-full border border-[#e8eaed] bg-white" aria-label="새로고침">
        <RefreshCw className={cn('h-4 w-4 text-[#626871]', loading && 'animate-spin')} />
      </button>
    </div>
  </div>;
};
...
const DecisionRow = ({ item, replay }: { item: DecisionTapeItem; replay: (item: DecisionTapeItem) => void }) => (
  <button type="button" onClick={() => replay(item)} className={cn(soft, 'w-full px-4 py-4 text-left')}>
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0">
        <div className="flex items-center gap-2"><span className="text-[14px] font-semibold text-[#1b1e24]">{item.market}</span><Pill color={tone(item.decision)}>{actionKo(item.decision)}</Pill></div>
        <div className="mt-2 line-clamp-2 text-[11px] leading-5 text-[#777e87]">{reasonKo(item.primaryReason ?? item.reasons?.[0])}</div>
        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[9px] text-[#a0a5ac]"><span>{dateTime(item.timestamp)}</span><span>{item.router?.route ?? item.strategyDisposition ?? 'strategy —'}</span><span>Council {item.council?.verdict ?? '—'}</span></div>
      </div>
      <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-[#b2b6bc]" />
    </div>
  </button>
);
...
const EvidenceRow = ({ item }: { item: OperationalEvidence }) => (
  <div className="border-b border-[#eef0f2] py-3 last:border-b-0">
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0"><div className="line-clamp-2 text-[12px] font-medium leading-5 text-[#2a2d33]">{item.title || item.rationale || 'Untitled evidence'}</div><div className="mt-1 text-[9px] text-[#9a9fa7]">{item.source || item.source_type || 'source unavailable'} · {item.observed_at ? dateTime(item.observed_at) : 'time unavailable'}</div></div>
      <Pill color={item.direction === 'BULLISH' ? green : item.direction === 'BEARISH' ? red : '#7f8790'}>{item.evidence_grade || item.direction || 'INFO'}</Pill>
    </div>
  </div>
);

const HomeView = ({ operations, decisions, replay, goPositions }: { operations: OperationsPayload | null; decisions: DecisionTapeItem[]; replay: (item: DecisionTapeItem) => void; goPositions: () => void }) => {
  ...
  return <main className="px-4 pb-28 pt-4">
    <section className={cn(card, 'overflow-hidden p-5')}>
      ...
      <div className="mt-2 text-[36px] font-semibold tracking-[-0.055em] text-[#101216]">{formatKrw(p?.equity)}</div>
      ...
      <div className="mt-4"><EquitySparkline points={operations?.equityCurve ?? []} /></div>
    ...
    <section className="mt-6"><Section title="Open positions" subtitle="현재 보유 중인 Paper 포지션" right={<button type="button" onClick={goPositions} className="text-[11px] font-semibold text-[#4c669e]">전체 보기</button>} />
      <div className="space-y-2">{(p?.openPositions ?? []).slice(0, 3).map((position) => <div key={position.market} className={cn(card, 'p-4')}><div className="flex items-start justify-between"><div><div className="text-[15px] font-semibold text-[#17191e]">{position.market}</div><div className="mt-1 text-[10px] text-[#999ea6]">opened {timeAgo(position.openedAt)}</div></div><Pill color={blue}>PAPER</Pill></div><div className="mt-4 grid grid-cols-3 gap-3"><Metric label="Average" value={formatKrw(position.averageCost)} /><Metric label="Stop" value={formatKrw(position.stopLossPrice)} accent={red} /><Metric label="Take profit" value={formatKrw(position.takeProfitPrice)} accent={green} /></div></div>)}{!p?.openPositions?.length && <div className={cn(card, 'p-5 text-[11px] text-[#8d9299]')}>현재 열린 포지션이 없습니다.</div>}</div>
    </section>
    ...
    <section className="mt-6"><Section title="System pulse" subtitle="메인에서는 핵심 상태만 보여줍니다." /><div className={cn(card, 'grid grid-cols-2 gap-x-4 gap-y-5 p-5')}><Metric label="Runtime" value={operations?.status ?? 'WAITING'} accent={tone(operations?.status)} /><Metric label="Open positions" value={p?.openPositions?.length ?? 0} /><Metric label="Evidence active" value={operations?.ingestion?.evidenceActive ?? '—'} /><Metric label="Latest route" value={latest?.router?.route ?? latest?.strategyDisposition ?? '—'} note={latest?.market ?? '—'} /></div></section>
  </main>;
};
...
const PositionsView = ({ operations }: { operations: OperationsPayload | null }) => {
  ...
  return <main className="px-4 pb-28 pt-4">
    ...
    <div className="space-y-3">{positions.map((position) => <section key={position.market} className={cn(card, 'p-5')}><div className="flex items-start justify-between"><div><div className="text-[18px] font-semibold tracking-[-0.025em] text-[#15171c]">{position.market}</div><div className="mt-1 text-[10px] text-[#999ea6]">opened {timeAgo(position.openedAt)}</div></div><Pill color={blue}>ACTIVE</Pill></div><div className="mt-5 grid grid-cols-2 gap-x-5 gap-y-5 border-t border-[#eef0f2] pt-4"><Metric label="Entry" value={formatKrw(position.entryPrice)} /><Metric label="Average cost" value={formatKrw(position.averageCost)} /><Metric label="Quantity" value={num(position.quantity, 5)} /><Metric label="Stop loss" value={formatKrw(position.stopLossPrice)} accent={red} /><Metric label="Take profit 1" value={formatKrw(position.takeProfit1Price ?? position.takeProfitPrice)} accent={green} /><Metric label="Take profit 2" value={formatKrw(position.takeProfit2Price)} accent={green} /></div></section>)}{!positions.length && <div className={cn(card, 'p-6 text-[11px] leading-5 text-[#8d9299]')}>현재 열린 포지션이 없습니다. 새로운 진입이 발생하면 이 화면에서 진입가·수량·SL/TP를 바로 확인할 수 있습니다.</div>}</div>
  </main>;
};
...
const ActivityView = ({ operations, decisions, replayDecision, replayTrade }: { operations: OperationsPayload | null; decisions: DecisionTapeItem[]; replayDecision: (item: DecisionTapeItem) => void; replayTrade: (item: ClosedTrade) => void }) => (
  <main className="px-4 pb-28 pt-4">
    <section><Section title="Decisions" subtitle="최근 판단과 canonical replay" /><div className="space-y-2">{decisions.slice(0, 8).map((item) => <DecisionRow key={`${item.market}-${item.timestamp}`} item={item} replay={replayDecision} />)}{!decisions.length && <div className={cn(card, 'p-5 text-[11px] text-[#8d9299]')}>최근 판단 기록이 없습니다.</div>}</div></section>
    <section className="mt-6"><Section title="Evidence" subtitle="최근 운영 payload로 들어온 근거" right={<span className="text-[10px] font-medium text-[#9ba0a7]">{operations?.ingestion?.evidenceActive ?? 0} active</span>} /><div className={cn(card, 'px-4')}>{(operations?.evidenceFlow ?? []).slice(0, 8).map((item) => <EvidenceRow key={item.id} item={item} />)}{!(operations?.evidenceFlow?.length) && <div className="py-5 text-[11px] text-[#8d9299]">표시 가능한 활성 Evidence가 없습니다.</div>}</div></section>
    <section className="mt-6"><Section title="Closed trades" subtitle="최근 실행 결과" /><div className="space-y-2">{(operations?.recentTrades ?? []).slice(0, 6).map((trade) => <button type="button" key={trade.id} onClick={() => replayTrade(trade)} className={cn(card, 'flex w-full items-center justify-between p-4 text-left')}><div><div className="text-[13px] font-semibold text-[#1b1e24]">{trade.market}</div><div className="mt-1 text-[9px] text-[#9a9fa7]">{dateTime(trade.closedAt)} · {short(trade.strategyVersion)}</div></div><div className="flex items-center gap-2"><div className="text-right"><div className="text-[13px] font-semibold" style={{ color: trade.netPnl >= 0 ? green : red }}>{formatKrw(trade.netPnl)}</div><div className="mt-1 text-[9px]" style={{ color: trade.netPnl >= 0 ? green : red }}>{percent(trade.returnPct)}</div></div><ChevronRight className="h-4 w-4 text-[#b5b9bf]" /></div></button>)}</div></section>
  </main>
);
...
export const BlackOracleMobileApp = () => {
  ...
  return <div className="min-h-[100dvh] text-[#111318]" style={{ background: page }}>
    <Header operations={operations} loading={loading} refresh={() => void load()} />
    {tab === 'home' && <HomeView operations={operations} decisions={decisions} replay={replayDecision} goPositions={() => setTab('positions')} />}
    {tab === 'portfolio' && <PortfolioView operations={operations} replay={replayTrade} />}
    {tab === 'positions' && <PositionsView operations={operations} />}
    {tab === 'activity' && <ActivityView operations={operations} decisions={decisions} replayDecision={replayDecision} replayTrade={replayTrade} />}
    {tab === 'more' && <MoreView operations={operations} factory={factory} events={sortedEvents} />}

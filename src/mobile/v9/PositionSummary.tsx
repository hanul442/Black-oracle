...
export const PositionSummary = ({ position, portfolioEquity, compact = false }: { position: OpenPosition; portfolioEquity?: number | null; compact?: boolean }) => {
  ...
  return <div className={`${card} ${compact ? 'p-4' : 'p-5'}`}>
    <div className="flex items-start justify-between gap-3">
      <div className="min-w-0"><div className="flex items-center gap-2"><span className="truncate text-[15px] font-semibold text-[#17191e]">{position.market}</span><span className="rounded-full border border-[#3767d62a] bg-[#3767d60b] px-2 py-0.5 text-[9px] font-semibold" style={{ color: blue }}>OPEN · PAPER</span></div><div className="mt-1 text-[9px] text-[#9299a2]">진입 {dateTime(position.openedAt)} · {timeAgo(position.openedAt)}</div></div>

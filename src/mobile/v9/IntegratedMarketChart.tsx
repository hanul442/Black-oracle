...
type ChartPayload = {
  ...
  error?: string;
  warning?: string;
  marketData?: { quality?: string; delayMinutes?: number; executionEligible?: boolean; researchOnly?: boolean };
};
...
const sourceLabel = (source?: string) => source === 'UPBIT_PUBLIC' ? 'Upbit Public' : source === 'KIS_OFFICIAL' ? 'KIS Official' : source === 'YAHOO_FINANCE' ? 'Yahoo Finance · delayed' : source || 'Market data';
  ...
  return <section>
    ...
    <div className="overflow-hidden rounded-[24px] border border-[#e7e9ed] bg-white">
      <div className="flex items-start justify-between gap-3 px-4 pb-2 pt-4">
        <div><div className="text-[9px] text-[#929aa2]">{sourceLabel(payload?.source)}</div><div className="mt-1 text-[27px] font-semibold tracking-[-0.045em]">{formatKrw(live)}</div><div className="mt-1 text-[10px] font-semibold" style={{ color: (periodReturn ?? 0) >= 0 ? green : red }}>{formatPercent(periodReturn, true)} · {frame.label}</div></div>
        <div className="text-right"><div className="text-[8px] text-[#9aa2aa]">Market as-of</div><div className="mt-1 text-[9px] font-medium text-[#59636d]">{dateTime(payload?.asOf ?? (candles.length ? candles[candles.length - 1].timestamp : null))}</div>{finance?.markPrice != null && <><div className="mt-2 text-[8px] text-[#9aa2aa]">Paper mark</div><div className="mt-1 text-[10px] font-semibold text-[#434950]">{formatKrw(finance.markPrice)}</div></>}</div>
      </div>
      {loading ? <div className="flex h-48 items-center justify-center text-[10px] text-[#8d939b]">실제 시장 차트를 불러오는 중…</div> : candles.length < 2 ? <div className="flex h-48 items-center justify-center px-5 text-center text-[10px] leading-5 text-[#8d939b]">{payload?.error || '실제 OHLCV 데이터가 없습니다. 임의 가격은 그리지 않습니다.'}</div> : <svg viewBox={`0 0 ${width} ${height}`} className="h-[190px] w-full touch-pan-y" role="img" aria-label={`${market} market chart`}>
      ...
      <div className="border-t border-[#eef1f3] px-4 py-3 text-[8px] leading-4 text-[#929aa2]">{payload?.marketData?.executionEligible === false ? '표시용/리서치 데이터 · 주문 권한 없음' : '시장 데이터'}{payload?.warning ? ` · ${payload.warning}` : ''}</div>
    </div>
  </section>;
};

...
import { actionKo, cn, pct, reasonKo, regimeKo, scoreText, strategyName, timeAgo } from './v2/types';
import { EmptyCard, EventCard, Header, Metric, Pill, ScoreGauge, Screen, SectionTitle, StatusChip } from './v2/ui';
import { formatKrw } from './v2/financial';
import { PortfolioPerformanceChart } from './v4/PortfolioPerformanceChart';
import { MarketUniverseTab } from './v5/MarketUniverseTab';

const MarketAnalysisV5 = lazy(() => import('./v5/MarketAnalysisV5').then((module) => ({ default: module.MarketAnalysisV5 })));
const PortfolioTabV4 = lazy(() => import('./v4/PortfolioTabV4').then((module) => ({ default: module.PortfolioTabV4 })));
const PositionDetailClarity = lazy(() => import('./v2/PositionDetailClarity').then((module) => ({ default: module.PositionDetailClarity })));
const StrategyDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.StrategyDetail })));
const CouncilDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.CouncilDetail })));
const EventDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.EventDetail })));
const EvidenceItemDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.EvidenceItemDetail })));
const EvidenceListDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.EvidenceListDetail })));
const LogDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.LogDetail })));
const TradeTimelineDetail = lazy(() => import('./v2/details').then((module) => ({ default: module.TradeTimelineDetail })));
...
const DetailLoading = () => (
  <div className="h-full overflow-y-auto bg-[#f7f8f9] px-4 pb-8 pt-[max(env(safe-area-inset-top),18px)]">
    <div className="h-9 w-32 animate-pulse rounded-xl bg-white" />
    <div className="mt-5 h-28 animate-pulse rounded-[22px] bg-white" />
    <div className="mt-3 h-52 animate-pulse rounded-[22px] bg-white" />
  </div>
);
...
const DataHealthCard = ({ operations, market, health, online }: { operations: OperationsPayload | null; market: string | null; health: MarketHealth; online: boolean }) => {
  ...
  const state = !online ? 'OFFLINE' : !runtimeOk ? stale ? 'STALE' : operations?.status ?? 'WAITING' : market && health.available === false ? 'MARKET DATA' : 'LIVE';
    ...
    <div className={cn('rounded-[20px] border p-4', good ? 'border-[#dbeee8] bg-[#f7fcfa]' : 'border-[#f0e5d6] bg-[#fffaf3]')}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-full', good ? 'bg-[#e4f6ef] text-[#0a8f6d]' : 'bg-[#fff0dc] text-[#a56c20]')}>{online ? <Wifi className="h-4 w-4" /> : <WifiOff className="h-4 w-4" />}</span>
          <div className="min-w-0"><div className="text-[12px] font-semibold text-[#37414a]">Data Health · {state}</div><div className="mt-1 truncate text-[11px] text-[#78838d]">Runtime {runtimeOk ? '정상' : stale ? '지연' : operations?.status ?? '대기'}{market ? ` · ${market}` : ''}</div></div>
        </div>
        {health.source && <span className="shrink-0 text-[10px] font-medium text-[#89939d]">{health.source}</span>}
      </div>
      {market && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-black/[0.04] pt-3 text-[10px] leading-4 text-[#87919a]"><span>{health.loading ? '가격 데이터 확인 중' : health.available ? '시장 데이터 정상' : '시장 데이터 확인 필요'}</span><span>최근 봉 {health.asOf ? timeAgo(health.asOf) : '—'}</span>{health.configured === false && <span>KIS 자격증명 미설정</span>}{health.error && <span className="w-full text-[#a56c20]">{health.error}</span>}</div>}
    </div>
  );
};
...
const RiskCard = ({ operations }: { operations: OperationsPayload | null }) => {
  ...
  return <div className="rounded-[22px] border border-[#edf0f2] bg-white p-4"><div className="flex items-center justify-between"><div className="text-[14px] font-semibold">현재 리스크</div><ShieldCheck className="h-5 w-5 text-[#69747e]" /></div><div className="mt-4 grid grid-cols-3 gap-3"><Metric label="오늘 손익률" value={pct(portfolio?.dailyPnlPct, true)} /><Metric label="현재 Drawdown" value={pct(portfolio?.currentDrawdownPct)} /><Metric label="포지션 노출" value={pct(exposure)} /></div><div className="mt-3 text-[11px] leading-5 text-[#87919a]">열린 포지션 {portfolio?.openPositions.length ?? 0}개 · Persisted Paper checkpoint 기준</div></div>;
};

const DecisionCard = ({ decision, market, onOpen }: { decision: DecisionTapeItem | null; market: string | null; onOpen: () => void }) => {
  if (!decision) return <button type="button" onClick={onOpen} className="block w-full text-left"><EmptyCard title={market ? `${market} 판단 Trace 없음` : '최신 판단 없음'} body="가격은 조회할 수 있지만 Strategy·Council·Risk 판단을 임의 생성하지 않습니다. 눌러서 실제 시장 차트를 확인하세요." /></button>;
  return <button type="button" onClick={onOpen} className="w-full rounded-[22px] border border-[#edf0f2] bg-white p-5 text-left shadow-[0_10px_30px_rgba(15,23,42,0.035)]"><div className="flex items-start justify-between gap-3"><div><div className="text-[12px] text-[#89939d]">{timeAgo(decision.timestamp)} · {regimeKo(decision.regime)}</div><div className="mt-1 text-[22px] font-semibold tracking-[-0.04em]">{decision.market}</div></div><StatusChip value={actionKo(decision.decision)} /></div><div className="mt-3 text-[13px] leading-6 text-[#66717b]">{reasonKo(decision.primaryReason || decision.reasons?.[0])}</div><div className="mt-4 grid grid-cols-4 gap-2 border-t border-[#f0f2f4] pt-4 text-center"><Metric label="Score" value={scoreText(decision.oracleTradeScore)} /><Metric label="Evidence" value={`${decision.evidenceActiveCount ?? 0}`} /><Metric label="Council" value={actionKo(decision.council?.verdict ?? '—')} /><Metric label="Risk" value={decision.riskDisposition ?? '—'} /></div></button>;
};

const HomeTab = ({ operations, decision, selectedMarket, health, online, loading, events, strategies, onRefresh, onAnalysis, onPortfolio, onEvidence, onCouncil, onLog, onMarket }: {
  ...
  const leader = [...strategies].sort((a, b) => (b.score ?? -1) - (a.score ?? -1))[0] ?? null;
  return <Screen>
    <div className="flex items-center justify-between pt-1"><div><div className="text-[25px] font-semibold tracking-[-0.05em]">Black Oracle</div><div className="mt-1 text-[12px] text-[#87919a]">Decision OS · Paper Operations</div></div><button type="button" onClick={onRefresh} aria-label="새로고침" className="flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-sm"><RefreshCw className={cn('h-5 w-5 text-[#5b6570]', loading && 'animate-spin')} /></button></div>
    {selectedMarket && <button type="button" onClick={onMarket} className="mt-4 flex w-full items-center justify-between rounded-2xl bg-[#eef3f6] px-4 py-3 text-left"><div><div className="text-[10px] font-medium text-[#80909a]">현재 연결된 시장</div><div className="mt-0.5 text-[15px] font-semibold">{selectedMarket}</div></div><span className="text-[11px] font-semibold text-[#60707c]">시장 변경 ›</span></button>}
    <div className="mt-4"><DataHealthCard operations={operations} market={selectedMarket} health={health} online={online} /></div>
    <div className="mt-4 rounded-[24px] border border-[#edf0f2] bg-white p-5 shadow-[0_16px_48px_rgba(15,23,42,0.05)]"><div className="text-[13px] font-medium text-[#7f8992]">총 자산 · Paper</div><div className="mt-2 text-[34px] font-semibold tracking-[-0.055em]">{formatKrw(portfolio?.equity)}</div><div className="mt-1 text-[17px] font-semibold" style={{ color: (totalReturn ?? 0) >= 0 ? '#0aa77d' : '#dc5a66' }}>{pct(totalReturn, true)}</div><div className="mt-4"><PortfolioPerformanceChart points={operations?.equityCurve ?? []} compact /></div><button type="button" onClick={onPortfolio} className="mt-4 flex w-full items-center justify-between border-t border-[#f0f2f4] pt-4 text-left"><span className="text-[13px] font-semibold text-[#3b454e]">Equity · Drawdown 상세</span><ChevronRight className="h-4 w-4 text-[#a2abb3]" /></button></div>
    <div className="mt-4"><RiskCard operations={operations} /></div>
    <section className="mt-7"><SectionTitle title={selectedMarket ? '선택 시장 Oracle 판단' : '현재 Oracle 판단'} action="차트 · 리포트" onAction={onAnalysis} /><DecisionCard decision={decision} market={selectedMarket} onOpen={onAnalysis} /></section>
    <section className="mt-7"><SectionTitle title="운영 모듈" /><div className="grid grid-cols-2 gap-3"><button type="button" onClick={onEvidence} className="rounded-[20px] border border-[#edf0f2] bg-white p-4 text-left"><Database className="h-5 w-5 text-[#56626d]" /><div className="mt-4 text-[14px] font-semibold">Evidence</div><div className="mt-2 text-[12px] leading-5 text-[#7f8992]">활성 {ingestion?.evidenceActive ?? 0} · 외부 {ingestion?.externalEvidenceActive ?? 0}<br />NARS 최근 {ingestion?.narsInboxRecent ?? 0}</div></button><button type="button" onClick={onCouncil} className="rounded-[20px] border border-[#edf0f2] bg-white p-4 text-left"><Bot className="h-5 w-5 text-[#56626d]" /><div className="mt-4 text-[14px] font-semibold">Council</div><div className="mt-2 text-[12px] leading-5 text-[#7f8992]">{decision?.council?.verdict ? actionKo(decision.council.verdict) : 'Trace 없음'}<br />찬성 {decision?.council?.approveCount ?? 0} · 반대 {decision?.council?.rejectCount ?? 0}</div></button><button type="button" onClick={onPortfolio} className="rounded-[20px] border border-[#edf0f2] bg-white p-4 text-left"><Briefcase className="h-5 w-5 text-[#56626d]" /><div className="mt-4 text-[14px] font-semibold">포지션</div><div className="mt-2 text-[12px] leading-5 text-[#7f8992]">열린 포지션 {portfolio?.openPositions.length ?? 0}개<br />현금 {formatKrw(portfolio?.cash)}</div></button><button type="button" onClick={onLog} className="rounded-[20px] border border-[#edf0f2] bg-white p-4 text-left"><ScrollText className="h-5 w-5 text-[#56626d]" /><div className="mt-4 text-[14px] font-semibold">Canonical Log</div><div className="mt-2 text-[12px] leading-5 text-[#7f8992]">현재 범위 {events.length}개<br />전략 선두 {leader?.name ?? '—'}</div></button></div></section>
  </Screen>;
};
...
const LogHubTab = ({ operations, events, selectedMarket, onRoute, onClear }: { operations: OperationsPayload | null; events: LedgerEvent[]; selectedMarket: string | null; onRoute: (route: DetailRoute) => void; onClear: () => void }) => <Screen><Header title="로그 · 감독" subtitle="선택 시장을 유지한 채 Evidence·Council·거래 trace를 추적합니다." />{selectedMarket && <div className="mb-4 flex items-center justify-between rounded-2xl bg-[#eef3f6] px-4 py-3"><div><div className="text-[10px] text-[#82909a]">현재 시장 필터</div><div className="mt-0.5 text-[14px] font-semibold">{selectedMarket}</div></div><button type="button" onClick={onClear} className="text-[12px] font-semibold text-[#60707c]">전체 보기</button></div>}<button type="button" onClick={() => onRoute('log')} className="w-full rounded-[22px] bg-[#121820] p-5 text-left text-white"><div className="flex items-center justify-between"><ScrollText className="h-5 w-5 text-white/75" /><ChevronRight className="h-4 w-4 text-white/45" /></div><div className="mt-4 text-[21px] font-semibold">Canonical Log</div><div className="mt-2 text-[12px] leading-5 text-white/65">{events.length}개 이벤트 · 검색·필터·상세 trace</div><div className="mt-4 text-[11px] text-white/45">Runtime {operations?.status ?? 'UNKNOWN'} · {operations?.mode ?? 'PAPER'}</div></button><section className="mt-7"><SectionTitle title="최근 이벤트" action="전체 로그" onAction={() => onRoute('log')} /><div className="space-y-3">{events.slice(0, 5).map((event) => <EventCard key={event.id || event.eventKey} event={event} onClick={() => onRoute('log')} />)}{!events.length && <EmptyCard title="로그 없음" body="현재 시장 범위에 해당하는 Canonical Ledger 이벤트가 없습니다." />}</div></section></Screen>;

const BottomNavigation = ({ tab, onChange }: { tab: Tab; onChange: (tab: Tab) => void }) => {
  const items: Array<{ id: Tab; label: string; Icon: React.ComponentType<{ className?: string }> }> = [{ id: 'home', label: '홈', Icon: Home }, { id: 'market', label: '시장', Icon: Search }, { id: 'strategies', label: '전략', Icon: ListTree }, { id: 'portfolio', label: '포트폴리오', Icon: Briefcase }, { id: 'more', label: '로그', Icon: ScrollText }];
...
const NoTraceDetail = ({ title, market, onBack }: { title: string; market: string | null; onBack: () => void }) => <div className="h-full overflow-y-auto bg-[#f7f8f9] px-4 pb-8 pt-[max(env(safe-area-inset-top),18px)]"><Header title={title} subtitle={market ? `${market}에는 해당 Canonical Trace가 없습니다.` : '시장 선택이 필요합니다.'} back={onBack} /><EmptyCard title="실제 Trace 없음" body="없는 Council·Risk·Trade 판단은 추정하거나 생성하지 않습니다. 시장 차트는 AI 리포트에서 확인할 수 있습니다." /></div>;
...
export const BlackOracleMobileApp: React.FC = () => {
  ...
  const selectedDecision = useMemo(() => selectedMarket ? decisions.find((item) => item.market === selectedMarket) ?? null : decisions[0] ?? null, [decisions, selectedMarket]);
  ...
  const detail = () => {
    ...
    if (route === 'analysis') return <MarketAnalysisV5 market={selectedMarket} decisions={decisions} onSelectMarket={setSelectedMarket} onCouncil={() => openChild('council', 'analysis')} onBack={back} />;
    ...
    if (route === 'council') return selectedDecision ? <CouncilDetail decisions={scopedDecisions} selected={selectedDecision} onSelect={(decision) => setSelectedMarket(decision.market)} onBack={back} /> : <NoTraceDetail title="Council" market={selectedMarket} onBack={back} />;
    if (route === 'trade') return (scopedDecisions.length || scopedTrades.length) ? <TradeTimelineDetail decisions={scopedDecisions} selected={selectedDecision} onSelectDecision={(decision) => setSelectedMarket(decision.market)} events={scopedEvents} recentTrades={scopedTrades} onSelectEvent={(event) => chooseEvent(event, 'trade')} onAnalysis={() => openChild('analysis', 'trade')} onBack={back} /> : <NoTraceDetail title="거래 추적" market={selectedMarket} onBack={back} />;
  ...
  return <div className="relative h-[100dvh] w-full overflow-hidden bg-[#f7f8f9] text-[#111418]" style={{ colorScheme: 'light' }}><AnimatePresence mode="wait"><motion.div key={route ?? tab} initial={{ opacity: 0, x: route ? 10 : 0 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: route ? -8 : 0 }} transition={{ duration: 0.16, ease: [0.16, 1, 0.3, 1] }} className="absolute inset-0"><Suspense fallback={<DetailLoading />}>{route ? detail() : tabView()}</Suspense></motion.div></AnimatePresence>{!route && <BottomNavigation tab={tab} onChange={switchTab} />}</div>;
};

...
if (!runtimeId) {
  ...
  const result = await exportCanonicalPaperEvents({ runtimeId, supabaseUrl, supabaseKey, pageSize: pageSize == null ? undefined : Number(pageSize), maxRows: maxRows == null ? undefined : Number(maxRows) });
  const payload = JSON.stringify({
    ...
    firstBoundary: result.firstBoundary,
    lastBoundary: result.lastBoundary,
    rows: result.rows,
  }, null, 2);
  if (outputPath === '-') process.stdout.write(`${payload}\n`);
  else {
    await writeFile(outputPath, `${payload}\n`, 'utf8');
    console.error(`Exported ${result.rows.length} canonical rows for ${runtimeId} at snapshot ${result.snapshotRecordedAt ?? 'empty'} (${result.snapshotFingerprint}) to ${outputPath}${result.truncated ? ' (TRUNCATED)' : ''}.`);
  }
}

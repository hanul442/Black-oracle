...
import { validateS0Baseline } from './verify-s0-baseline.mjs';

const fixture = () => JSON.parse(readFileSync(new URL('../ops/s0-baseline.json', import.meta.url), 'utf8'));

test('S0 baseline validates without claiming release readiness', () => {
  const result = validateS0Baseline(fixture());
  assert.equal(result.contractValid, true);
  assert.equal(result.releaseReady, false);
  assert.equal(result.s0Status, 'IN_PROGRESS');
...
for (const [name, change] of [
  ...
  ['false S0 completion', m => { m.s0Status = 'VERIFIED'; }],
  ...
  test(`rejects ${name}`, () => {
    ...
    change(manifest);
    assert.throws(() => validateS0Baseline(manifest));
  });
}

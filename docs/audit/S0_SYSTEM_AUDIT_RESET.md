# BLACK ORACLE S0 — System Audit & Reset Baseline

Status: **IN_PROGRESS**  
Observed: 2026-09-18  
Canonical baseline: `55be5181d807bae3f4044c1cb3d39650506266f3`  
Governing documents: Product Constitution v2 + Master Sprint Plan v3

## 1. Purpose

S0 freezes interpretation before implementation resumes.
...
4. Which blockers must close before S1/S2 can safely build on the system?
...
| Service | Latest observed deploy SHA | Config source | Status | S0 interpretation |
| --- | --- | --- | --- | --- |
...
S0 must treat these jobs as live production actors even when a corresponding Railway service has not recently written a checkpoint.
...
This may be intentional for service-role-only tables; it is **not** automatically a security defect. Before exposing any of these tables through future product Data API paths, S0/S1 must verify explicit grants and access intent.

Performance advisor reports many unused indexes. **Do not remove them during S0.** Low observed usage may reflect low sample size, shadow pipelines, or future query paths.
...
| **ABSORB** | PR #176 baseline/validator concepts into S0; useful PR #175 product research already captured by Constitution v2; older Monitor/Audit/Positions ideas into AutoTrade + Replay where still useful |
| **RETIRE** | Constitution v1 as active authority; Master Plan v2 as active roadmap; old top-level Case/Hypothesis/Scenario UX; duplicate Log/Ledger product surfaces; old navigation specifications |
...
- **#176** — old B0 baseline: **ABSORB**. Its offline safety-contract idea is reimplemented here under S0.
- **#177** — competing B0-B9 master plan: **ABSORB / RETIRE**. Commercial/safety rules already reconciled into the merged v3 plan.
...
S0 may move to GATE_REVIEW only when:
...
- offline S0 invariant validator passes.

Until then: **S0 IN_PROGRESS / releaseReady=false**.

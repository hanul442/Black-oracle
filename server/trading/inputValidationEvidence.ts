...
const overallDisposition = (records: InputValidationLedgerRecord[]): MarketInputValidationEvidence['disposition'] => {
  ...
  return records.length === REQUIRED_PROMOTION_TIMEFRAMES.length ? 'PASS' : 'INSUFFICIENT_DATA';
};
...
export const buildMarketInputValidationEvidence = async (
  ...
  const disposition = overallDisposition(records);

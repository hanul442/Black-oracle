...
test('projects NARS pipeline activity and evidence coverage request', () => {
  const events = buildEvidenceAndEquityCanonicalEvents({
    ...
    evidenceOps: {
      importedBeforeConsume: 1,
      consumedPackets: 2,
      importedAfterConsume: 3,
      acquisitionRequests: 1,
      acquisitionSourcesIngested: 2,
      errors: [],
    },
    markets: [{
      timestamp: 1_788_934_590_000,
      market: 'KRW-WLD',
      decision: 'NO_TRADE',
      evidenceActiveCount: 0,
      strategyDisposition: 'TREND_MOMENTUM',
      reasons: ['Evidence coverage request req-wld-1 queued for NARS acquisition/re-analysis.'],
    }],
  }, 'black-oracle-paper');
  ...
  const events = buildEvidenceAndEquityCanonicalEvents({
    ...
    equityCycle: {
      ...
      decisions: [{
        ...
        technicalScore: 71,
        evidenceScore: 64,
        evidenceIds: ['nars:packet-1:KRX-000660'],
        reasons: ['Research-only equity gate remains closed.'],
      }],
    },
  }, 'black-oracle-paper');
